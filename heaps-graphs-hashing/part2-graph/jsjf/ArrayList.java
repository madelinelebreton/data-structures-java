...
public abstract class ArrayList<T> implements ListADT<T>, Iterable<T>
{
	...
	{
		T[] larger = (T[])(new Object[list.length*2]); // create new larger list object

		// copy list into the larger list
		for(int i=0; i<list.length; i++)
		{
			larger[i] = list[i];
		}
		list = larger; // replace the list with the larger version
	}
	...
	public T removeLast() throws EmptyCollectionException
	{
		...
		if(isEmpty())
		{
			throw new EmptyCollectionException("ArrayList"); // throw an error if the list is already empty
		}

		T result = list[rear-1]; // save item to return
		list[rear-1] = null; // remove last element
		rear--; // decrement last element variable
		modCount++; // for error tracking
		
		return result; // return the last element, which was removed
	...
	public T removeFirst() throws EmptyCollectionException
	{
		...
		T result = list[0]; // save element to return
		...
		list[rear-1] = null; // remove what used to be the last element
		rear--; // keep track of last element in list
		modCount++; // for error tracking

		return result; // return the (now removed) first element
	...
	public T first() throws EmptyCollectionException
	{
		...
		if(isEmpty())
		{
			throw new EmptyCollectionException("ArrayList"); // throw an error if the list is empty
		}
	...
	public T last() throws EmptyCollectionException
	{
		if(isEmpty())
		{
			throw new EmptyCollectionException("ArrayList"); // throw error if list is empty
		}

		return list[rear-1]; // last element in the list
	}
	...
	public boolean isEmpty()
	{
		...
		return(rear == 0); // true if the list is empty, false otherwise
	}
	...
	{
		// To be completed as a Programming Project
		return rear; // number of elements in the list
	}
	...
	public String toString()
	{
		...
		String result = "["; // new variable to store result
		for(int i=0; i<rear; i++)
		{
			result += list[i]; // add each element to the result
			if(i<(rear-1))
			{
				result += ","; // comma seperators except on last element
			}
			result += "]"; // to finish the string
		}
		return result; // return string representation of list
	...
	private class ArrayListIterator implements Iterator<T>
	{
		...
		/* 
		 * Returns the element at the requested position in the list
		 */
		public T get(int elementNum) throws EmptyCollectionException
		{
			if(elementNum < 0 || elementNum >= rear)
			{
				throw new IndexOutOfBoundsException(); // invalid index error
			}

			return list[elementNum]; // return element at the requested position
		}
	}	
}
