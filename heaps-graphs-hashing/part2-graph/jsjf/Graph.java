...
public class Graph<T> implements GraphADT<T>
{
    ...
    {
        removeEdge(getIndex(vertex1), getIndex(vertex2)); // use removeEdge method by finding the indices
    }
    ...
    public void removeEdge(int index1, int index2)
    {
        if (indexIsValid(index1) && indexIsValid(index2))
        {
            adjMatrix[index1][index2] = false; // set the edges to false
    ...
    {
        addVertex(null); // use addVertex() method with null object associated
    }
    ...
    {
        int index = getIndex(vertex); // get index to use removeVertex(index) method
        if(index != -1) // if the vertex exists
            removeVertex(index); // remove the associated vertex
    }
    ...
    public void removeVertex(int index)
    {
        if(!indexIsValid(index))
            return; // invalid method call, no index to be returned
        ...
        for (int l = index; l < numVertices - 1; l++)
            for (int m = 0; m < numVertices; m++)
                adjMatrix[l][m] = adjMatrix[l][m + 1];

        numVertices--; // remove vertex from counter
        modCount++; // for error handling
        }
    ...
    public Iterator<T> iteratorBFS(int startIndex)
    {
        ...
        traversalQueue.enqueueRear(startIndex);
        visited[startIndex] = true;
        
        while (!traversalQueue.isEmpty())
        {
            x = traversalQueue.dequeueFront();
                                        ...
                                        (int startIndex, int targetIndex)
    {
        ...
        {
            index = (traversalQueue.dequeueFront()).intValue();
    ...
    public int size()
    {
        return numVertices; // integer num of vertices in the graph
    }
    ...
    {
        return (numVertices == 0); // true if empty
    }
    ...
    {
        if (isEmpty()) // nothing in the graph
            return false;

        // run BFS from vertex 0 using existing method
        Iterator<T> bfsIter = iteratorBFS(0);

        int count = 0; // counter for number of connected vertices
        while (bfsIter.hasNext()) // iterate through all 
        {
            bfsIter.next();
            count++;
        }

        return count == numVertices; // true if all vertices are connected
    }
    ...
    {
        for (int i = 0; i < numVertices; i++) // loop through each vertex
        {
            if (Objects.equals(vertices[i], vertex)) // if the target vertex is found
    ...
    {
        return index >= 0 && index < numVertices; // valid indices are positive and within range of vertices
    }
    ...
    {
        return Arrays.copyOf(vertices, numVertices); // use copyOf method from Arrays
    }
